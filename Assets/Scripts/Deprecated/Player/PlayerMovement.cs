...
public class PlayerMovement : MonoBehaviour {
	...
	void Update () {
		if (Input.GetButton ("Vertical")) {
			...
			transform.Translate (0, Input.GetAxis ("Vertical") *  creature.getProp("moveSpeed") * Time.deltaTime, 0);
		}

		if (Input.GetButton ("Horizontal")) {
			transform.Rotate (-1 * Input.GetAxis ("Horizontal") * transform.forward * creature.getProp("rotateSpeed") * Time.deltaTime);
		}
	...
	bool RotateToTarget(Vector2 heading) {
		...
		transform.rotation = Quaternion.RotateTowards(transform.rotation, qTo, creature.getProp("rotateSpeed") * Time.deltaTime);
	...
	bool MoveToTarget() {
		...
		if(RotateToTarget (heading) && heading.magnitude > 0)
			col.attachedRigidbody.velocity = heading / heading.magnitude * creature.getProp("moveSpeed");
